<div align="center">
  ...
  <h1 align="center">
    ONDEWO S2T Client C-Sharp
  </h1>
</div>

## Overview

`Ondewo.S2T.Client` is a compiled version of the
[ONDEWO S2T API](https://github.com/ondewo/ondewo-s2t-api) — the gRPC interface to ONDEWO
Speech-to-Text — generated with the
...
over the `ondewo-s2t-api` submodule, and it is regenerated in full by `make build`.

## Installation

The package is published to [nuget.org](https://www.nuget.org/packages/Ondewo.S2T.Client) as
**`Ondewo.S2T.Client`**. No custom feed, credential or `nuget.config` entry is needed — the default
`nuget.org` source is enough.

With the .NET CLI, from the directory of the project that should consume it:

```shell
dotnet add package Ondewo.S2T.Client
```
...
- **Versioning.** `Ondewo.S2T.Client` **7.5.x** is generated from ONDEWO S2T API **7.5.0**: major
  and minor always match the API, the patch number is this client's own.

From source:

```shell
git clone https://github.com/ondewo/ondewo-s2t-client-csharp.git   ## Clone the repository
cd ondewo-s2t-client-csharp                                        ## Change into the repo directory
...
// `package ondewo.s2t;` becomes `Ondewo.<Pascal>` — check the `namespace` line at the top of
...
├── ondewo-s2t-api                               <----- submodule: the .proto sources
├── ondewo-proto-compiler                    <----- submodule: the code generator
├── Ondewo.S2T.Client.csproj     <----- generated project file (tracked)
...
| `generate_ondewo_protos`              | runs that image over `ondewo-s2t-api/ondewo` and writes the library back into the repo  |
...
- The generated `Ondewo.S2T.Client.csproj` is **tracked**. On the next run the image finds it in the
...
The same glob is why `Ondewo.S2T.Client.csproj` carries
...
Bump `ONDEWO_S2T_VERSION` in the `Makefile`, add a `RELEASE.md` entry in the existing format under a
`## Release ONDEWO S2T Csharp Client <version>` heading — `build_gh_release` slices the release notes out by
...
Pushing a version tag (`7.5.0`, `7.5.0-rc.1` — the shape `make create_release_tag` writes) also triggers
