...
Almost everything in this repository is generated. `api/` and `Ondewo.S2T.Client.csproj` are the
output of the `ondewo-csharp-proto-compiler` docker image running over the `ondewo-s2t-api` submodule, and they are
overwritten in full by `make build`.

- A change to the **API surface** (a message, a field, an RPC) belongs in
  [ondewo-s2t-api](https://github.com/ondewo/ondewo-s2t-api), not here. Once it is released there, bump
  `ONDEWO_S2T_API_GIT_BRANCH` in the `Makefile` and run `make build`.
